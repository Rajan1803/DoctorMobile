...
class MessageAdapter: RecyclerView.Adapter<MessageAdapter.MessageHolder>() {
    class MessageHolder(val binding: RecyclerMessageItemBinding): RecyclerView.ViewHolder(binding.root) {
        fun bind(message: Message) {
            binding.message = message
            binding.imgvPicture.setImageResource(message.image)
            if (message.unseenMessages != 0) {
                binding.btnUnreadMsg.visibility = View.VISIBLE
                binding.btnUnreadMsg.text = message.unseenMessages.toString()
                binding.txtvMessage.setTextColor(binding.root.context.resources.getColor(R.color.skyBlue))
            }else {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): MessageHolder {
        return MessageHolder(RecyclerMessageItemBinding.inflate(LayoutInflater.from(parent.context),parent,false))
    }
